...
export default async function syncOnce({ userConfigPath, postsDir, outPath }) {
  ...
  // postImage
  if (site.images?.['post-image']) {
    const name = path.basename(site.images['post-image']);
    css += `  --bg-postImage: url("/assets/${name}");\n`;
  }

  // postImageMobile
  if (site.images?.['post-image-mobile']) {
    const name = path.basename(site.images['post-image-mobile']);
    css += `  --bg-postImage-mobile: url("/assets/${name}");\n`;
  }
